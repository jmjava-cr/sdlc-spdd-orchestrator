...
| Schema via `DataDictionary` | ✅ `SpddEntityDictionary` (also used to validate `findByLabel` input) |

Test evidence: 32 unit tests green in guide `com.embabel.guide.spdd`
(16 service incl. root-guard/idempotency/lesson edges, 8 controller status mapping,
8 MCP tool JSON contract).
