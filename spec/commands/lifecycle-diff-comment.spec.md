...
family: lifecycle
slug: diff-comment
copilot_description: Generate a PR review comment draft from the current diff vs base (does not post).
copilot_mode: agent
claude_description: Generate a PR review comment draft from the current diff vs base (does not post).
claude_argument_hint: "[short message] [WORK-ID]"
---
...
Generate a short PR review comment draft from the current diff versus the merge base. Do not post the comment. Do not implement code.
---END---
